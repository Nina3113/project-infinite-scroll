...
export default function Profile({ navigation, route }) {
  ...
  const getUser = () => {
    axios
      .get(
        `http://sweeftdigital-intern.eu-central-1.elasticbeanstalk.com/user/${route.params.id}`
      )
      .then((res) => {
        setUser(res.data);
  ...
  const getFriends = (id) => {
    ...
    axios
      .get(
        `http://sweeftdigital-intern.eu-central-1.elasticbeanstalk.com/user/${id}/friends/${currentPage}/15`
      )
      .then((res) => {
  ...
  const renderItem = ({ item }) => {
    return (
      ...
      >
        <Image
          style={styles.image}
          source={{ uri: `${item.imageUrl}?v=${item.id}` }}
        />
        <Text
          style={styles.textName}
        >{`${item.prefix} ${item.name} ${item.lastName}`}</Text>
  ...
  const listHeader = () => {
    return (
      <View>
        <Image
          style={styles.image}
          source={{ uri: `${user.imageUrl}?v=${user.id}` }}
        />
        <Text
          style={styles.textName}
        >{`${user.prefix} ${user.name} ${user.lastName}`}</Text>
        ...
        <View>
          ...
          >{`Company: ${company.name} ${company.suffix}`}</Text>
